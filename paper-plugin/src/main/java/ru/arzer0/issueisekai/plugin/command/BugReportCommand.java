...
public final class BugReportCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        Player target = target(sender, args, Bukkit::getPlayerExact, messages);
        if (target != null) {
            open(target);
        }
        return true;
    ...
    static Player target(
        ...
        if (args.length == 0) {
            ...
            sender.sendMessage(messages.component("message.usage-console"));
            return null;
        }
        if (args.length != 1) {
            sender.sendMessage(messages.component("message.usage-player"));
            return null;
        }
        if (!sender.hasPermission("bugreport.open.others")) {
            sender.sendMessage(messages.component("message.no-permission-other"));
            return null;
        }
        Player target = onlinePlayer.apply(args[0]);
        if (target == null) {
            sender.sendMessage(messages.component(
                    "message.player-offline", Placeholder.unparsed("player", args[0])));
        }
        return target;
    }
    ...
    public void submit(Player target, String category, String description) {
        ...
        if (!result.accepted()) {
            target.sendMessage(messages.component(result.error()));
            return;
        }
        ...
        if (eventResult.cancelled()) {
            target.sendMessage(messages.component("message.cancelled"));
            return;
        }
        result = validator.validate(
                ...
                target.hasPermission("bugreport.cooldown.bypass"));
        if (!result.accepted()) {
            target.sendMessage(messages.component(result.error()));
            return;
        }
                ...
                .runTask(
                        ...
                        () -> {
                            ...
                            if (error == null) {
                                ...
                                online.sendMessage(messages.component("message.queued"));
                            } else {
                                online.sendMessage(messages.component("message.queue-failed"));
                            }
                        }));
    }
